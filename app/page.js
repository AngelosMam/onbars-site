...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-cyan-500/30">
      ...
      {/* BACKGROUND GLOWS - Προσθέτουν βάθος στο site */}
  ...
  whileHover={{ 
    scale: 1.25, 
    rotate: [0, -2, 2, 0], // Ένα μικρό "τρέμουλο" (shake)
    filter: "drop-shadow(0 0 30px rgba(59, 131, 246, 0.84))" // Δυναμώνει το Aura glow
  }}
...
>
  ...
  />
        ...
        >
          <a href="#beta" className="bg-white text-black px-8 py-4 rounded-2xl font-bold hover:bg-gray-200 transition-all transform hover:scale-105 active:scale-95 shadow-xl shadow-white/10">
            Download Android
          </a>
          <a href="#beta" className="bg-zinc-900 border border-zinc-700 text-white px-8 py-4 rounded-2xl font-bold hover:bg-zinc-800 transition-all transform hover:scale-105 active:scale-95">
            Download iOS
          </a>
        </motion.div>
      </section>

      {/* STATS STRIP - 65,000+ PARKS */}
      <motion.div 
  ...
  {[
    ...
    { src: "/screen3.png", color: "from-purple-500 to-pink-400" },
    { src: "/screen4.png", color: "from-orange-500 to-purple-400" }
  ].map((item, i) => (
    <motion.div 
      key={i}
      initial={{ opacity: 0, y: 20 }}
      ...
      whileHover={{ y: -15 }} // Το κινητό σηκώνεται λίγο παραπάνω στο hover
      className="relative group cursor-pointer"
    >
      {/* Το χρωματιστό Glow πίσω από το κινητό */}
      ...
      <section id="beta" className="max-w-4xl mx-auto px-6 py-32 text-center">
        ...
        >
          ...
          <p className="text-gray-400 mb-10 text-lg">
            Limited spots available for early athletes. Get your unique Aura handle.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              placeholder="Enter your email"
              className="flex-1 px-6 py-4 rounded-2xl bg-zinc-800/50 border border-zinc-700 text-white outline-none focus:border-white transition-all"
            />
            <button className="bg-white text-black px-8 py-4 rounded-2xl font-bold hover:bg-cyan-400 transition-all">
              Join
            </button>
          </div>
        </motion.div>
